...
- **Measured trigger, a falsified constraint.** If a worker reports that it
  ...
  Re-spawn at `worker-opus-high`, naming the constraint, the reason, and the
  evidence the worker found, and stating that the constraint is void. This is
  the one escalation with a measurement behind it: on the benchmark task built
  to this shape (T10), sonnet at every effort level found the reason false,
  said so, and obeyed the constraint anyway, and `worker-opus-high` cleared
  the task nine of nine from a cold start (D42, D44). The cheaper move, voiding
  the constraint yourself and re-running the floor with it lifted, is
  unmeasured; if you take it, record that you did.
