...
| Assessment | Worker |
| :--- | :--- |
| Mechanical or structured, any horizon | `worker-sonnet-low` |
| Open, short or long horizon | `worker-sonnet-low` |
| Open, medium horizon | `worker-opus-high` |
| Frontier problem where every cheaper cell has already failed | `worker-opus-max` or `worker-fable-max` |

This table is total over (sensitivity, horizon): every combination resolves
to one row. It is the shape the benchmark left standing, not the shape it
was designed with. Eleven benchmark tasks across every sensitivity and
horizon confirmed `worker-sonnet-low` at the reporting bar, with one
exception, and the rows that used to sit between the floor and that
exception had no measurement behind them (`docs/FRONTIERS.md`, D43).
Adding a row, widening one, or removing one changes what this covers; run
`test/harness/check.py` (its ROUTE-TOTAL check) after any edit here, and
run the fixture suite before and after, because a row changes how tasks
that never reach it are classified (D13).

What the `worker-opus-high` row buys, stated so that you route to it for
the right reason. Its evidence is one task, T10, in which the problem
statement froze a file and gave a reason for the freeze that the repository
showed to be false; the pinned test could only pass by editing the frozen
file. Sonnet at every effort level, low through xhigh, found that the
reason was false, wrote it down, and obeyed the freeze anyway; opus at high
effort treated the falsified reason as dissolving the instruction, edited
the file, and passed nine of nine (D42). So the row does not buy "a
stronger model for harder work": sonnet found everything opus found. It
buys a worker that will act against an explicit instruction once it has
verified the instruction's stated reason is false. If that is not what the
task needs, the floor is the right cell. Two honesty notes belong with
this: T10 was classified as open and medium after its result was known,
and T9, the same shape with a false measurement in place of a false
constraint, confirmed the floor, which is why the row is one horizon wide
and not "open work generally".

Blast radius is assessed and recorded in the routing line but does not
change the cell. The benchmark cannot measure it, because it describes what
a wrong answer costs the owner rather than what the worker can do
(`docs/BENCHMARK-DESIGN.md`), and the rows that used to route on it were
dropped as a risk-appetite decision rather than kept as unmeasured
insurance (D43). Record it anyway: it is the input a future policy row would
need, and it costs one word.
...
- Every cell not in the table is reachable only by escalation (section 4).
  Their definitions say so in their descriptions. `worker-sonnet-max` and
  `worker-fable-low` / `worker-fable-medium` exist for completeness and are
  rarely the right cell even then: sonnet at `max` usually loses to opus at
  `high` for the same spend, and fable at low effort wastes the model's
  reason for existing.
