...
| `claude -p` has a `--max-budget-usd <amount>` flag, "Maximum dollar amount to spend on API calls (only works with --print)" | `claude -p --help` (free, no session started) | E25. Not previously used anywhere in this repository; `benchmark.py` and `score_routing.py` enforce cost only by counting `total_cost_usd` after each call returns. `system_controller.py` passes it on every role call as a second, platform-enforced backstop beside the Controller's own Python-side running total, since which of the two would fire first, or whether the flag aborts mid-call cleanly, is unverified |
...
| `--permission-mode acceptEdits` and `--allowedTools` passed to the top-level `claude -p` forwarder propagate to a worker it spawns via the Task tool, the same way documented auto-mode classifier behaviour does | `test/harness/benchmark.py`'s FORWARDER_PERMISSION_ARGS | `docs/en/permission-modes` confirms this for auto mode's classifier explicitly but says nothing either way for acceptEdits or dontAsk; a worker report of a permission denial despite the flag being set would falsify it, a worker that edits files without one would confirm it |
