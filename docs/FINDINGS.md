...
| `score_routing.py` runs against a live install; its `claude -p --output-format json` field names (`result`, `total_cost_usd`) are correct | Sonnet-orchestrator run against `orchestrator-scratch`, 17/17 fixtures parsed, cost reported as USD 0.7623 | E12 (sonnet half); see `test/results/2026-09-05-routing.md` for the full table |
