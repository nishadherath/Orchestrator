...
| A sonnet orchestrator scored higher fixture agreement than an opus orchestrator on the same 17 fixtures, at a fraction of the cost: sonnet 11/17 (USD 0.7623), opus 8/17 (USD 3.5808), one run each | Opus's disagreements were overwhelmingly not misjudgment: on F01, F09, F11, F15 and F17 it assessed all three axes the same as the human fixture, then chose `action: clarify` instead of `action: spawn` anyway (9 of 17 opus verdicts were `clarify`, versus 3 of 17 for sonnet). Sonnet's disagreements were mostly axis misjudgment (under-provisioning), not excess caution | E12 (both halves); answers open question 6 with one run per model, evidence in `test/results/2026-09-05-routing-{sonnet,opus}.md`. Whether opus's caution is a scoring artefact (the fixtures assume `spawn` is correct wherever a human would spawn) or a genuinely worse fit for this orchestrator role is a judgement call, not settled by this data alone |
