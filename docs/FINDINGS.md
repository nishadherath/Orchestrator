...
| A spawned subagent's cost and token usage roll up into the parent `claude -p --output-format json` session's `total_cost_usd` (and any `usage` field) | `test/harness/benchmark.py`, whose whole cost measurement depends on it | Spawn a worker on a task with a distinctive, large token cost, compare the parent's reported `total_cost_usd` against the same call with no spawn; a session's own `/cost` reading taken immediately after the spawn is a second, independent check |
