...
TASK = (
    "Write a detailed technical explainer, about 3000 words, of how TCP "
...
SPAWN_PROMPT = (
    "This is a diagnostic run, not a real request. Spawn exactly one worker "
    "with subagent_type `worker-sonnet-low`, using the Task tool, and "
    "nothing else: do not do the task yourself, do not spawn any other "
    "worker, do not ask a clarifying question, and do not pass a model "
    "parameter when spawning.\n\n"
...
def main(argv: list[str]) -> int:
    ...
    rows: list[dict] = []
    for condition, prompt in (("SPAWN", SPAWN_PROMPT), ("DIRECT", DIRECT_PROMPT)):
        for rep in range(1, args.reps + 1):
            print(f"running {condition} rep {rep}/{args.reps}...", file=sys.stderr)
            row = run_once(project, prompt, args.model, args.timeout)
            row["condition"] = condition
            row["rep"] = rep
            rows.append(row)
            cost = row["total_cost_usd"]
            print(f"  total_cost_usd={cost} elapsed_s={row['elapsed_s']} num_turns={row['num_turns']}",
                  file=sys.stderr)

    spawn_costs = [r["total_cost_usd"] for r in rows if r["condition"] == "SPAWN" and r["total_cost_usd"] is not None]
    direct_costs = [r["total_cost_usd"] for r in rows if r["condition"] == "DIRECT" and r["total_cost_usd"] is not None]
    ...
    print("\n=== summary ===")
    print(f"SPAWN mean total_cost_usd:  {spawn_mean}")
    print(f"DIRECT mean total_cost_usd: {direct_mean}")
    ...
    if args.record:
        ...
        out = RESULTS_DIR / f"{dt.datetime.now().strftime('%Y-%m-%d')}-cost-rollup-check.md"
        lines = [
            f"# Cost roll-up check, {dt.datetime.now().strftime('%Y-%m-%d %H:%M')}",
            "",
            f"Project: `{project}`. Model: {args.model}. Reps per condition: {args.reps}.",
            "",
            "| Condition | Rep | total_cost_usd | num_turns | elapsed_s |",
            "| :--- | :--- | :--- | :--- | :--- |",
        ]
        for r in rows:
            lines.append(f"| {r['condition']} | {r['rep']} | {r['total_cost_usd']} | {r['num_turns']} | {r['elapsed_s']} |")
        lines += [
            "",
            ...
            "",
            "```json",
            json.dumps([{"condition": r["condition"], "rep": r["rep"], "usage": r["usage"]} for r in rows], indent=2),
            "```",
        ]
        out.write_text("\n".join(lines) + "\n", encoding="utf-8")
        print(f"\nwrote {out}")
