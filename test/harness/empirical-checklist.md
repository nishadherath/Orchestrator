...
| E22 | Time and price a minimal schema-forced classification call: `claude -p` at `worker-sonnet-low` with a short task, asked for three enumerated axis values and nothing else, ten times, recording `total_cost_usd` each time | Mean and spread of cost per verdict | The `R` term the whole cost thesis turns on (`docs/PREMISES.md`, dissolution check). At USD 0.1645 the table can never beat B0; at USD 0.02 it needs a 12 percent floor-failure rate. Stages 5 and 6 need the real figure before choosing a mechanism |
