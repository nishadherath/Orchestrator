...
acceptance criteria (proposed if absent, then fixed), and B0. Executable
integrity-v1 freezes a caller-supplied acceptance-v2 contract before this step;
the Framer must reproduce its criteria exactly. If no external contract exists,
the first valid Frame criteria become the provisional frozen criteria.
...
Controller as one of its two model calls. The executable quick path does make
that bounded classification call. Integrity-v1 stops with a gap before
generation when it returns unstable; `legacy-quick-v0` records the result but
continues for historical replay.
...
Premise operation: search.
Executable integrity-v1 then applies code-owned eligibility to the Selector's
rank order and exclusions. A winner needs current-ledger evidence, exactly one
passing non-derivable critique, no unresolved load-bearing premise and no
unverified introduced premise. B0 must meet the same material checks and still
be named by the current Frame. If no candidate qualifies, Close writes a gap.
...
Quick mode does not run this step: falsification is on paper only. Integrity-v1
uses the eligibility rule in Select and returns a gap when no candidate is
defensible. The earlier first-survivor-else-B0 rule exists only as the explicit
`legacy-quick-v0` replay policy.
Premise operation: add true premises.
...
premises, the next cheapest test). Executable integrity-v1 also writes a
digest-bound `controller-evidence.json` with outcome, readiness, frozen
acceptance, verified findings, rejected hypotheses, remaining uncertainty,
artefact hashes and accounting. The full design additionally specifies the
library write-back: the
