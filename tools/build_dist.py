...
Every `dist/ORCHESTRATOR-REFERENCE.md` build strips every `<!-- rationale:start -->
...
Bundle version: {version}. Generated by tools/build_dist.py from {source}.
...
# Files that make the directory a usable package but are not copied into a
...
def planned_files(version: str, dist_dir: Path = DIST, with_rationale: bool = False) -> dict[Path, str]:
    ...
    `ORCHESTRATOR-REFERENCE.md` (docs/PLAN-2.md Stage 4, D64); the default strips
    ...
    core = (SRC / "ORCHESTRATOR_CORE.md").read_text(encoding="utf-8")
    orchestrator_body = routing.rstrip("\n") + "\n\n" + lifecycle if with_rationale else core
    source = "src/ROUTING.md and src/LIFECYCLE.md" if with_rationale else "src/ORCHESTRATOR_CORE.md"
    ...
    payload = {}
    for path, content in sorted(out.items(), key=lambda item: item[0].as_posix()):
        relative = path.relative_to(dist_dir).as_posix()
        if relative not in INSTALL_SUPPORT_FILES:
            payload[relative] = hashlib.sha256(content.encode("utf-8")).hexdigest()
    manifest = {
        "schema_version": 1,
        ...
        "configuration": {
            ...
            "owned_values": ["promptCacheTtl", "autoCompactWindow", "statusLine", "subagentStatusLine"],
            "owned_arrays": ["permissions.allow", "hooks.SessionStart"],
            "optional_mcp_value": "mcpServers.graft",
            "claude_markers": ["<!-- orchestrator:begin -->", "<!-- orchestrator:end -->"],
