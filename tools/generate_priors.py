...
def beta(passes, n, cap=CAP, kind="measured", prov=""):
    m = (passes + 1) / (n + 2)
    n_eff = min(n, cap)
...
bucket("mechanical", "short", "contained", beta(18, 18, prov=F + "T1 confirmed 9 of 9 twice (04d2acc original and replication); fixtures F01, F02, F15"))
bucket("mechanical", "medium", "contained", beta(36, 36, cap=4, kind="bracketed", prov=F + "no task at this triple; T1 (short) and T2 (long) both 9 of 9 twice, D19 bracket; fixture F03; n_eff capped at 4"))
bucket("mechanical", "long", "contained", beta(18, 18, prov=F + "T2 confirmed 9 of 9 twice"))
bucket("structured", "short", "contained", beta(18, 18, prov=F + "T3 confirmed 9 of 9 twice; fixture F04"))
bucket("structured", "medium", "contained", beta(36, 36, cap=4, kind="bracketed", prov=F + "no task at this triple; T3 and T4 bracket; fixture F05; n_eff capped at 4"))
bucket("structured", "long", "contained", beta(18, 18, prov=F + "T4 confirmed 9 of 9 twice after the D16/D17 grader fix; fixture F07"))
bucket("open", "short", "contained", beta(27, 27, prov=F + "T5 9 of 9 twice, T8 9 of 9 after D30's regrade; fixture F09"))
bucket("open", "medium", "contained",
       beta(9, 12, prov=F + "T9 9 of 9 at the floor (a false measurement), T10 0 of 3 at the floor (a false constraint, D42); the triple cannot separate them, D42 calls T10 a disposition frontier; fixture F10"),
       rungs={"worker-sonnet-xhigh": beta(0, 12, prov="T10: sonnet-xhigh 0 of 12 given floor failure (D42); the intermediate sonnet rungs are not in the default ladder for this reason"),
              "worker-opus-high": beta(12, 12, prov="T10: opus-high 12 of 12 given floor failure (D42, the only confirmed cell above the floor)")})
bucket("open", "long", "contained", beta(36, 36, prov=F + "T6 9 of 9 twice, T7 9 of 9 (after D20), T11 9 of 9; fixture F13"))

for key in list(buckets):
    s, h, _ = key.split("/")
    src = buckets[key]["floor"]
    inh = beta(src["passes_measured"], src["n_measured"], cap=3, kind="policy-inherited",
...
(REPO / "src" / "routing_priors.json").write_text(json.dumps(doc, indent=2, ensure_ascii=False) + "\n", encoding="utf-8", newline="\n")
for k, b in buckets.items():
    f = b["floor"]
    r = b["rungs_given_failure_below"].get("worker-opus-high")
    print(f"{k:32} floor mean {f['mean']:.3f} n_eff {f['n_eff']:2} {f['kind']:16} | opus-high|fail {r['mean']:.2f} {r['kind']}")
